...
def init_endpoints(gift_router: APIRouter):
	...
	async def get_booked(user: AuthUserDep, cursor: int = None, limit: int = 24) -> FullGiftsListSerializer:
		...
		return JSONResponse(
			FullGiftsListSerializer.model_validate({
			'items': gifts
			},
	...
	async def create(wishlist_id: int, payload: СreateGift, user: AuthUserDep) -> FullGiftSerializer:
		...
		return JSONResponse(
			FullGiftSerializer.model_validate(gift,
			from_attributes=True).model_dump(context={
			'auth_user_id': user.id
			})
		)
	...
	@gift_router.post(
		...
		responses={
		404: {
		'description': 'in case if gift does not exist'
		},
		403: {
		'description': 'in case if updater has no permission for his action'
		}
		}
	)
	...
	@gift_router.put(
		...
		403: {
		'description': 'in case if booker has no permission for his action'
		}
		}
	)
	...
	@gift_router.patch(
		...
		403: {
		'description': 'in case if user has no permission for his action'
		}
		}
	)
