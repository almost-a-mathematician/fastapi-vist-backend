...
class SearchService:
	...
	async def search(self, search: str, user: User):
		async with self.Session() as session:
			...
			user_query = (
				select(User).where(func.similarity(User.username,
				search) > 0.01).order_by(func.levenshtein(User.username,
				search)).limit(10)
			)

			wishlist_query = (
				select(Wishlist).where(func.similarity(User.username,
				search) > 0.01).order_by(func.levenshtein(Wishlist.name,
				search)).options(selectinload(Wishlist.users),
				selectinload(Wishlist.gifts)).limit(20)
			)
