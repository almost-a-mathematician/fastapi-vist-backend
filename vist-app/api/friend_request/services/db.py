...
class FriendRequestService:
    ...
    async def create(self, id: int, user: User, status: str):
        async with self.Session() as session:
            ...
            does_request_exist = select(FriendRequest).where(
                or_(
                    (FriendRequest.receiver_id == id) & (FriendRequest.sender_id == user.id),
    ...
    async def update(self, id: int, user: User, status: str):
        async with self.Session() as session:
            ...
            if status == 'accepted':
                if (friend_request.status == 'sent' and friend_request.receiver_id == user.id) or (
                friend_request.status == 'rejected' and friend_request.rejected_by_id == user.id):
                    friend_request.status = status
                else:
                    raise FriendRequestPermissionException

            elif status == 'rejected':
                if friend_request.status == 'accepted' or (
                friend_request.status == 'sent' and friend_request.receiver_id == user.id):
                    friend_request.status = status
