...
def init_endpoints(friend_request_router: APIRouter):
	...
	async def get(user: AuthUserDep) -> FriendRequestsListSerializer:
		...
		return JSONResponse(
			FriendRequestsListSerializer.model_validate({
	...
	@friend_request_router.post(
		...
		404: {
		'description': 'in case if receiver does not exist'
		}
		}
	)
	async def create(user: AuthUserDep, payload: CreateUserFriend) -> FriendRequestSerializer:
		...
		return JSONResponse(
			FriendRequestSerializer.model_validate(friend_request,
			from_attributes=True).model_dump(context={
			'auth_user_id': user.id
			})
		)
	...
	@friend_request_router.delete(
		...
		404: {
		'description': 'in case if friend request does not exist'
		}
		}
	)
