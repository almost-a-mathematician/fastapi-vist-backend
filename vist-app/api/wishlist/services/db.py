...
class WishlistService:
	...
	def filter_visible_for(self, query, user: User):
		query = query.where(
			...
			(Wishlist.archived_at == None)
			| (Wishlist.owner_id == user.id)  # перегрузка оператора 
		).where(~Wishlist.users.any() | Wishlist.users.any(User.id == user.id))
		return query
	...
	async def get(self, user: User, owner_id: int, cursor: int | None, limit: int):
		async with self.Session() as session:
			...
			query = (
				...
				selectinload(Wishlist.gifts)).order_by(Wishlist.id).limit(limit)
			)
	...
	async def get_by_friends(self, user: User, cursor: int | None, limit: int):
		async with self.Session() as session:
			...
			query = (
				select(Wishlist).join(User,
				Wishlist.owner_id == User.id).join(user_friend_association,
				User.id == user_friend_association.c.friend_id).where(
				user_friend_association.c.user_id == user.id
				).options(selectinload(Wishlist.users),
				selectinload(Wishlist.gifts)).order_by(Wishlist.id).limit(limit)
			)
	...
	async def create(self, owner: User, name: str, archived_at=None):
		async with self.Session() as session:
			...
			wishlist = Wishlist(name=name, owner=owner, archived_at=archived_at, users=[owner], gifts=[])
