...
def init_endpoints(wishlist_router: APIRouter):
	...
	async def get(user: AuthUserDep, owner: int, cursor: int = None, limit: int = 24) -> WishlistsListSerializer:
		...
		return JSONResponse(
			WishlistsListSerializer.model_validate({
			'items': wishlists
			},
	...
	async def create(payload: CreateWishlist, creator: AuthUserDep) -> WishlistSerializer:
		...
		wishlist = await wishlist_service.create(name=payload.name, owner=creator, archived_at=payload.archived_at)

		return JSONResponse(
			WishlistSerializer.model_validate(wishlist,
			from_attributes=True).model_dump(context={
			'auth_user_id': creator.id
			})
		)
	...
	@wishlist_router.put(
		...
		responses={
		404: {
		'description': 'in case if wishlist does not exist'
		},
		403: {
		'description': 'in case if updater is not the owner'
		}
		}
	)
	...
	@wishlist_router.delete(
		...
		403: {
		'description': 'in case if deleter is not the owner'
		}
		}
	)
	async def delete(id: int, user: AuthUserDep):
		try:
			await wishlist_service.delete(id, user)
		except WishlistIsNotExistException:
			raise HTTPException(status_code=404)
		except WishlistPermissionException:
			raise HTTPException(status_code=403)
