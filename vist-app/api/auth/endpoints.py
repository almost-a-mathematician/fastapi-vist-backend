...
def init_endpoints(auth_router: APIRouter):
	...
	@auth_router.post(
		path='/register', responses={
		409: {
		'model': DuplicateUserSerializer
		}
		}
	)
	async def register(payload: Register) -> RegisterSerializer:
		...
		try:
			user = await user_service.create(username=payload.username, password=password_hash, email=payload.email)
		except UserExistsException as e:
			return JSONResponse(status_code=409, content=DuplicateUserSerializer(column=e.column).model_dump())
		...
		return JSONResponse(
			RegisterSerializer.model_validate({
			'user': user,
			'avatar_token': access_token.create(id=user.id, lifetime=1)
			},
